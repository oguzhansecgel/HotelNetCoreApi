...
namespace HotelProject.WebUI.Controllers
{
	public class TestimonialController : Controller
	{
		...
		public async Task<IActionResult> Index() // listeleme metodu
		{
			...
			var responserMessage = await client.GetAsync("http://localhost:5185/api/Testimonial");
		...
		public async Task<IActionResult> AddTestimonial(TestimonialViewModel model)
		{
			...
			var responseMessage = await client.PostAsync("http://localhost:5185/api/Testimonial", stringContent);
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"http://localhost:5185/api/Testimonial/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"http://localhost:5185/api/Testimonial/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
